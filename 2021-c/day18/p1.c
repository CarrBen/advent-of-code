...
int main(int argc, char* argv[])
{
	...
	struct node *sum = malloc(1 * sizeof(struct node));
	...
	if(lineCount == 1) {
		...
		for(int i=1; i<lineCount; i++) {
			...
			// Not worrying about memory leaks
			struct node *oldResult = result;
			result = malloc(sizeof(struct node));
			result->leftNode = oldResult;
			result->rightNode = input[i];

			printf("Reducing %d\n", i);
			printNumber(result, true);
			int rr = reduce(result);
			while(rr > 0) {
				rr = reduce(result);
			}
			printf("Reduced %d\n", i);
